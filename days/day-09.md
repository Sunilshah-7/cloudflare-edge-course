...
async function fetchJWKS(jwksUrl) {
  ...
  await env.JWKS_CACHE.put(jwksUrl, JSON.stringify(keys), {
    expirationTtl: 3600,
  });
  return keys;
}
...
1. **Cloudflare**: [JWT Example](https://developers.cloudflare.com/rules/snippets/examples/jwt-validation/) (~5 min)
